...
void zanalysis(double th, const emxArray_real_T *z_a, emxArray_real_T *val, emxArray_real_T
               *pos)
{
  int i5;
  int i;
  ...
  while (i <= z_a->size[0] - 1) {
    ...
    b_findpeaks(th, c_z_a, tp_pos, p_pos);
    if (tp_pos->size[1] < 2) 
    {
        b_findpeaks(1, c_z_a, tp_pos, p_pos);
    }
    if (tp_pos->size[1] >= 2) {
