...
bool Data::loadFromFile(std::string filename) {
  ...
  // Open input file
  // changed here: filename is actual the stringstream
  /*
...
*/
    std::stringstream input_file, header_file;
    input_file.str(filename);
    header_file.str(filename);
  ...
  getline(header_file, header_line);

  // Find out if comma, semicolon or whitespace seperated and call appropriate method
    {
    result = loadFromFileWhitespace(header_file, header_line);
  }
...
bool Data::loadFromFileWhitespace(std::stringstream& input_file, std::string header_line) {
